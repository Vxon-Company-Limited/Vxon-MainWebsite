...
import { beforeAfter } from "@/lib/translations/product-claw";

export function BeforeAfter({ locale }: { locale: string }) {
  const ba = t(beforeAfter, locale);
  return (
    <section className="py-24 px-6 relative z-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            <h3 className="text-lg font-bold text-zinc-400 mb-6">{ba.left.title}</h3>
            <div className="space-y-4">
              {ba.left.items.map((item) => (
