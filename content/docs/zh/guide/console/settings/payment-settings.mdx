...
- type: 通道标识，用于后端路由与下单。
  ...
  - 详细逻辑见后端控制器 `controller/topup.go`（参考: [controller/topup.go](https://github.com/Vxon/vxon/blob/main/controller/topup.go)）。
...
- 详细实现逻辑见后端控制器 [controller/topup.go](https://github.com/Vxon/vxon/blob/main/controller/topup.go)
