...
description: AstrBot 配置教程 — 将开源 Agent 聊天机器人平台对接 Vxon，为 QQ、飞书、钉钉、企业微信等即时通讯注入 AI 能力。
---
...
AstrBot 支持接入 Vxon 作为模型提供商，用户可以通过 Vxon 来访问和使用各种 AI 模型服务。

### 配置步骤

#### 获取 Vxon API Key 密钥

在 Vxon 注册并登录后，点击上方导航栏的「控制台」，点击「令牌管理」，然后点击「添加令牌」按钮，创建一个新的 API Key 密钥，选择适当的权限，然后点击「创建」。
...
#### 在 AstrBot 中配置 Vxon 服务提供商

打开 AstrBot 管理面板，进入「模型提供商」页面，然后，点击「新增模型提供商」按钮。

Vxon 完美地支持了 OpenAI Chat Completion 和 Responses 接口，我们点击 「OpenAI」，进入 OpenAI 提供商的配置页面。

在弹出的对话框中，将 API Base URL 设置为 Vxon 的接口地址。如果您本地部署了 Vxon，则填写本地地址，例如 `http://localhost:3000/v1`，如果您使用第三方服务商提供的 Vxon 服务，则填写相应的 URL 地址，例如 `https://api.example.com/v1`。

然后，将 API Key 填入「API Key」字段中，点击「保存」按钮。

![astrbot-provider-config](/assets/astrbot/image-2.png)

然后点击保存，完成 Vxon 提供商的配置。

#### 应用服务提供商

进入「配置文件」页面，找到模型一节，将「默认聊天模型」修改为刚刚创建的 Vxon 提供商，点击「保存」按钮。

![apply](/assets/astrbot/image-3.png)

至此，您已经成功配置了 Vxon 作为 AstrBot 的模型提供商。现在，您可以通过 AstrBot 来访问和使用 Vxon 提供的各种 AI 模型服务了。
