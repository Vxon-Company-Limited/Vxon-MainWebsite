...
本文档提供了在本地环境中设置和开发 Vxon 项目的详细步骤，适合希望参与项目开发或进行功能定制的开发者。
...
Vxon 支持通过 `.env` 文件配置环境变量。创建 `.env` 文件（可从 `.env.example` 复制）：
...
```bash
PORT=3000
SQL_DSN=root:password@tcp(localhost:3306)/vxon   # 如使用MySQL，取消注释并修改
...
Vxon 的前端代码位于 `web` 目录中，使用 React 和 [semi design 组件库](https://semi.design/zh-CN) 开发。
...
1. 查看 [GitHub Issues](https://github.com/Vxon/vxon/issues)
