...
description: AionUi setup guide — free, open-source desktop office agent supporting Gemini CLI, Claude Code, Codex, and more. Connect to Vxon for multi-model AI workflows.
---
...
## Vxon Configuration Steps

1. **Copy API key in Vxon**
...
4. **Configure API Information**
   - API Address: Fill in your Vxon site address (format: `https://<your-vxon-domain>/v1`)
   - API Key: Paste the API Key copied from Vxon console

5. **Add Models**
   - Select the model to add from the dropdown
   - Model name should match the model name configured in Vxon
   - Select the appropriate request protocol

6. **Start Using**
   - Return to the chat page
   - Select the configured Vxon model to start a conversation
