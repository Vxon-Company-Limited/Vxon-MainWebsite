...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with Vxon.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with Vxon.
</Callout>
...
LangBot supports integrating with locally deployed Vxon and third-party Vxon services built using Vxon.

### Usage

1.  Obtain an API key from Vxon
    ![获取 API key](/assets/langbot/get_api_key.png)

        If Vxon is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party Vxon service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select Vxon as the provider, and fill in the corresponding API key and API address
...
LangBot supports using Vxon's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select Vxon as the provider
