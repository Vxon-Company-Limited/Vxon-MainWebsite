...
Ratio settings are a core configuration of the Vxon billing system, allowing flexible control over the billing standards for various models and user groups by setting different ratios.

## Ratio System Overview

Vxon uses a three-tier ratio system to calculate user quota consumption:
...
In the Vxon system, ratios are key parameters for calculating quota consumption. Quota is an internal billing unit of the system, and all API calls are ultimately converted into quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by Vxon)
...
Vxon adopts a dual billing mechanism of pre-consumption and post-consumption:
