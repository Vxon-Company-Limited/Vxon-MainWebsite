...
This document provides detailed steps for deploying Vxon using Docker Compose.
...
1. Create a directory for Vxon deployment:
...
- **Logs for a specific service** (Examples: `vxon`, `mysql`, `redis`)
...
- **View logs using container name** (when `container_name` is set, e.g., `vxon` in the configuration)
